...
def find_beautiful_languages(languages)
  ...
  hash.select { |key, val| val[:is_beautiful?] == true }
end
...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  languages[language_name].find do |k, v|
    next if k != :facts # if key is not facts array, skip current iteration

    return v[fact_index] # if parameter fact_index is defined
  end

  # official solution was simple:
