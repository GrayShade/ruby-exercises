def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  # omitting the second optional argument from Array.new & instead using a
  # block to create an immutable array of mutable objects. Thus changing in
  # one nested array won't make changess in other nested arrays:

  # using nil below is unnecessary as per official solution.
...
def delete_seat_from_row(chart, row_index, seat_index)
  ...
  # not using << delete >>, as it requires the element to be specified for
  # deletion as far as I understand. Thats why it was unable to find obj to
  # be deleted. << row_at >> uses index as we want here.
...
def find_favorite(array_of_hash_objects)
  ...
  hash = {}
  array_of_hash_objects.any? do |ele|
    hash = ele if ele[:is_my_favorite?] == true
  end
  hash.empty? ? nil : hash
